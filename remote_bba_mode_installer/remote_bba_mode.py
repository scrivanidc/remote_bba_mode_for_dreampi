...
def is_dreampi_active():
    try:
        ...
        # Any non-zero exit (e.g. failed, inactive) means it's not active — so we can proceed
        return False
...
def monitor_dns_activity():
    global active_ip, session, dns_rule_applied, traffic_rule_applied, session_closed, last_traffic_time, last_traffic_line
    while running:
        if is_dreampi_active():
            if dns_rule_applied:
